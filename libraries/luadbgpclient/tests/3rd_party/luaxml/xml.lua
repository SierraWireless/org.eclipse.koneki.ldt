...
xmlParser = function(handler)     
    ...
    obj.parse = function(self, str, parseAttributes)
        ...
        while match do
            ...
            elseif string.sub(tagstr,1,8) == "![CDATA[" then
                ...
                while 1 do
                    if string.find(tagstr,self._TAGFINAL) or 
                       string.find(tagstr,self._ATTR1FINAL) or 
                       string.find(tagstr,self._ATTR1FINAL) then
                        break
                    end
                    extstart,extend,endt2 = string.find(str,self._TAGEXT,endmatch+1)
                    tagstr = tagstr .. string.sub(str,endmatch,extstart-1)
                    if not match then 
                        self:_err(self._errstr.xmlErr,pos)
                    end 
                    endmatch = extend
    ...
    obj._ATTRERR1   = '=%s*"[^"]*$'
    obj._ATTRERR2   = '=%s*\'[^\']*$'
    obj._ATTR1FINAL = '[%w-:_]+%s*=%s*"[^"]*"%s*$'
    obj._ATTR2FINAL = '[%w-:_]+%s*=%s*\'[^\']*\'%s*$'
    obj._TAGFINAL   = '^%s*%S-%s*$'
    obj._TAGEXT     = '(%/?)>'
